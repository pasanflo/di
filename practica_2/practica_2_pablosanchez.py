...
class MainWin:
	...
	def __init__(self):
		...
		self.widgets  = gtk.glade.XML("practica_2_pablosanchez.glade")
	...
	def on_button2_clicked(self, bbdd): #grabar
		...
		c.execute('insert into tabla(usuario,contrasena,email,nombre,apellido,descripcion) values ("'+str(usuario)+'","'+str(contrasena)+'","'+str(email)+'","'+str(nombre)+'","'+str(apellido)+'","'+str(descripcion)+'")')
		conex.commit()
